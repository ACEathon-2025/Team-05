...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const getCurrentUser = async () => {
      ...
      // Load analysis history if user is authenticated
      if (session?.user) {
        try {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      ...
      // Reload analysis history when auth state changes
      if (session?.user) {
        try {
          setIsLoading(true);
          const { history } = await analysisHelpers.getAnalysisHistory();
  ...
  const handleAnalysis = () => {
    // Prefer captured image (base64), then file base64, then object URL
    const baseImage = capturedImage || previewBase64;
    if (baseImage) {
      navigate('/analysis', { state: { image: baseImage } });
      return;
    }

    // If we only have an object URL (previewUrl), convert it to base64 before navigating
    if (previewUrl) {
      // Attempt to fetch the object URL and convert to base64 synchronously
      fetch(previewUrl)
        .then((res) => res.blob())
        ...
        .then((dataUrl) => {
          // set previewBase64 so future analysis can reuse
          setPreviewBase64(dataUrl);
          navigate('/analysis', { state: { image: dataUrl } });
        })
        .catch((err) => {
          console.error('Failed to convert object URL to base64 before navigation', err);
          // fallback to sending object URL (may fail if revoked)
          navigate('/analysis', { state: { image: previewUrl } });
        });
      return;
    }

    // No image available
    navigate('/analysis', { state: { image: null } });
  };
  ...
  const handleCapture = () => {
    ...
    setShowCameraModal(false);
    stopCameraStream();
    // store and navigate using base64 data URL
    navigate('/analysis', { state: { image: dataUrl } });
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPreviewUrl(url);
    // Also read file as base64 for sending to backend
    const reader = new FileReader();
    reader.onload = () => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              {analysisHistory.length > 0 ? (
                ...
                [
                  ...
                  { id: '3', date: '2025-01-05', severity: 'High', score: 45 }
                ].map((scan) => (
                  <div 
                    key={scan.id} 
                    className="glassmorphism rounded-xl p-4 md:p-6 hover:bg-purple-900/10 transition-all duration-300 cursor-pointer"
                  ...
                  >
                    ...
                    <div className={`text-xs md:text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} flex items-center`}>
                      <Clock className="w-3 h-3 md:w-4 md:h-4 mr-1" />
                      {scan.date}
                      <span className="ml-2 text-xs text-purple-400">(Sample)</span>
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            {analysisHistory.length > 0 && (
              <p className="text-green-400 text-sm">
                {analysisHistory.filter(item => {
                  ...
                  const now = new Date();
                  const thisMonth = date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
                  return thisMonth;
                }).length} this month
              </p>
            )}
          ...
          <div className="glassmorphism rounded-xl p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Latest Score</p>
                <p className="text-xl md:text-2xl font-bold gradient-text">
                  {analysisHistory.length > 0 ? analysisHistory[0].score : '--'}
                </p>
              </div>
            ...
            {analysisHistory.length > 1 && (
              <p className={`text-sm ${
                analysisHistory[0].score >= analysisHistory[1].score 
                  ? 'text-green-400' 
                  : 'text-red-400'
              }`}>
                {analysisHistory[0].score >= analysisHistory[1].score 
                  ? `+${analysisHistory[0].score - analysisHistory[1].score}` 
                  : `-${analysisHistory[1].score - analysisHistory[0].score}`
                } from previous
              </p>
            )}
          </div>
          ...
          <div className="glassmorphism rounded-xl p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Analysis Streak</p>
                {analysisHistory.length > 0 ? (
                  <p className="text-xl md:text-2xl font-bold gradient-text">
                    {(() => {
                      // Calculate streak
                      ...
                      for (let i = 0; i < analysisHistory.length; i++) {
                        const scanDate = new Date(analysisHistory[i].date);
                        const scanDay = new Date(scanDate.getFullYear(), scanDate.getMonth(), scanDate.getDate());
                        const daysDiff = Math.floor((today.getTime() - scanDay.getTime()) / (1000 * 60 * 60 * 24));
                  ...
                  <p className="text-xl md:text-2xl font-bold gradient-text">0</p>
                )}
              </div>
              <Clock className="w-6 h-6 md:w-8 md:h-8 text-purple-400" />
            </div>
            <p className="text-green-400 text-sm">
              {analysisHistory.length > 0 ? "Keep it up!" : "Start your first scan!"}
            </p>
          </div>
        </div>
      </div>
